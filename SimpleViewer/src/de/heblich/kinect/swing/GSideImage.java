...
public class GSideImage extends JPanel implements KinectReaderEvent{
	...
	private void renderImage(UserTrackerFrameRef userTrackerFrameRef, KinectReader kinectReader){
		...
		if(userTrackerFrameRef.getUsers().size() > 0){
			...
			if(skeleton.getState() == SkeletonState.TRACKED){
				...
				drawCross(g, center);
				g.drawString(""+ud.getId(), 320+convertZ(center), 240-center.getY()*scaler);
	...
	private void drawLine(Graphics2D g, Point3D<Float> p1, Point3D<Float> p2) {
		float nz1 = convertZ(p1);
		float nz2 = convertZ(p2);
		Line2D l = new Line2D.Float(320+nz1, 240-p1.getY()*scaler, 320+nz2, 240-p2.getY()*scaler);
		g.draw(l);
	}
	
	private void drawCross(Graphics2D g, Point3D<Float> point) {
		float nz = convertZ(point);
		Line2D l1 = new Line2D.Float(320+nz-3, 240-point.getY()*scaler, 320+nz+3, 240-point.getY()*scaler);
		Line2D l2 = new Line2D.Float(320+nz, 240-point.getY()*scaler-3, 320+nz, 240-point.getY()*scaler+3);
		
		g.draw(l1);
		g.draw(l2);
	}

	private float convertZ(Point3D<Float> point) {
		return point.getZ() * 320 / 1500-200;
	}
}
